...
/**
 * User and Hero relathinship class
 */
class UserHero {
  /**
   *
  ...
  /**
   * get all the herores relates to the user
  ...
  /**
   * get all favorites heroes from a user
   * @param {number} id id of the user
   * @return {array} if the function is successful it will return one or more elements from the DB, if not will be returned indicating that there has been a failure
   */
  async getFav (id) {
  ...
  async bestMarverHero () {
    return await _DB.consulta(
      `select a.idHero, a.heroName, a.image, ROUND(sum(b.score) / count(a.idHero),1) as score FROM heroes a, usu_hero b  WHERE a.publisher = 'Marvel Comics' AND a.idHero = b.idHero GROUP by idHero ORDER by score DESC`
    )
  }
  /**
  ...
  async bestDCHero () {
    return await _DB.consulta(
      `select a.idHero, a.heroName, a.image, ROUND(sum(b.score) / count(a.idHero),1) as score FROM heroes a, usu_hero b  WHERE a.publisher = 'DC Comics' AND a.idHero = b.idHero GROUP by idHero ORDER by score DESC`
    )
  }

  /**
 * Get the most followed heros
 * @return {array} if the function is successful it will return and array elements from the DB, if not will be returned indicating that there has been a failure
 * 
 */
  async mostFollowHeros () {
    return await _DB.consulta(
      `select a.idHero, a.heroName, a.image, count(a.idHero) as followers FROM heroes a, usu_hero b WHERE a.idHero = b.idHero AND b.follow = 1 GROUP by idHero ORDER by followers DESC LIMIT 10`
    )
  }

  /**
   * Get a hero and a useer related
   * @param {number} idUser id of the user
   * @param {number} idHero id of the Hero
...
/**
 * Get all hero comment
 * @return {array} if the function is successful it will return and array elements from the DB, if not will be returned indicating that there has been a failure
 * 
 */
  async getHeroComments (idHero) {
    return await _DB.consulta(
      `SELECT UH.comment, UH.score,U.alias,U.idUsu,U.photo from usu_hero UH JOIN users U ON(U.idUsu = UH.idUsu) where idHero = ${idHero}`
    )
  }

  /**
   * Get the hero rate score
